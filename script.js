...
function playNewAkteChime() {
  if (!canPlayAkteChime()) return;
  try {
...
async function fetchData({ force = false } = {}) {
  ...
  try {
    ...
    const rows = tableRows.map((row) => {
      ...
      if (DASHBOARD_HEADER_LABELS.has(lower)) return false;
      return /\d/.test(row.Eingang);
    });
